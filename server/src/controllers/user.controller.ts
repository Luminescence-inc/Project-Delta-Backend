...
import { getJwtToken, jwtTokenSecret } from "../utils/jwt.util";
    ...
    registerUser = async(req: z.infer<typeof RegisterRequestSchema>, res: Response)=>{
        ...
        try {
            ...
            if(user.uuid){
                ...
                const newVerification =  await this.userService.createUserVerification(user.uuid, hashedUniqueString, Date.now(), Date.now()+21600000, VerificationType.EMAIL);
                ...
                if(newVerification.uuid){
                    ...
                    return respond.status(200).success(true).code(200).desc("successful Signed Up!").responseData({token}).send()
                }
            }
        } catch (error) {
            console.error(error);
            ...
            if(userDetails?.password){
                ...
                if(!compareSync(password, userpassword)){
                    ...
                    if(!userDetails?.verified){
                        ...
                        const hashedUniqueString  =  hashSync(uniqueString, 10); // env: salt for verification
                        const previousEmailVerificationLink = await this.userService.findVerificationLinkByUserId(userDetails.uuid, VerificationType.EMAIL);
                        ...
                        if(previousEmailVerificationLink){
                            if(previousEmailVerificationLink.expiresUtc < new Date()){
                                ...
                                const newVerification =  await this.userService.createUserVerification(userDetails.uuid, hashedUniqueString, Date.now(), Date.now()+21600000, VerificationType.EMAIL);
                                if(newVerification){
    ...
    verifyUserEmail = async(req: any, res: Response)=>{
        ...
        try {
           ...
           if(!verificationLinks){
                return respond.status(400).success(false).code(499)
                .desc("Account has been verified or link has expired. logIn to generate new verification link or Check your email for a new verification link")
                .send();
           }else{
                ...
                if(expiresUtc < new Date()){
                    ...
                    return respond.status(400).success(false).code(499).desc("Verification link has expired. logIn to generate new verification link or Check your email for a new verification link").send();
                }
    ...
    generateUserVerificationOrPassLink = async(req: any, res: Response, type: string)=> {
        ...
        try {
            ...
            if(userDetails){
                ...
                const newVerification =  await this.userService.createUserVerification(id, hashedUniqueString, Date.now(), Date.now()+21600000, verificationType);
                ...
                if(newVerification){
                    if(type==="verification"){
                        ...
                        .then(()=>{
                            ...
                            return respond.status(200).success(true).code(200).desc(`Verification email sent to: ${userDetails.email}`).send();
                        })
                        .catch((err)=>console.error(`Verification email failed with error message - ${err}`))
                    }
                    if(type==="password"){
                    ...
                    return respond.status(400).success(false).code(400).desc("Verification email failed. Email link wasn't generted").send();
                }
            }
        } catch (error) {
            console.error(error);
