      ...
      .then(emailStatus => {
        ...
        user
          .then(data => {
            ...
            const newVerification = this.userService.createUserVerification(
              data.uuid,
              hashedUniqueString,
              Date.now(),
              Date.now() + 21600000
              ...
              .then(() => {
                generateVerificationEmail(data.uuid, data.email, uniqueString, Date.now() + 21600000)
                  ...
                  .catch(err => {
                    return res
                      .status(400)
                      .send({ success: false, message: `Verification email failed with error message - ${err}` });
                  });
              })
              .catch(err => {
                res
      ...
      .then(links => {
        if (links.length > 0) {
          links.forEach(link => {
            ...
            if (expiresUtc < new Date()) {
              ...
              if (compareSync(uniqueString, dbHashedUniqueString)) {
                userVerified = true;
                this.userService.verifyUserById(userId).then(() => {
          ...
          if (userVerified) {
            ...
            return res
              .status(401)
              .send({ success: false, message: 'Verification Link has expired or Invalid verification Link.' });
          }
        } else {
          return res.status(400).send({
            success: false,
            message:
              "Account has been verified or it doesn't exist or link has expired. Sign up or log in or generate new verification link",
          });
        }
      })
      .catch(err => {
