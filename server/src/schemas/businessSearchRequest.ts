...
import { BusinessProfileFilterField, SortBy, SortDirection } from "@src/enums/business.enum";
...
export const BusinessSearchRequestSchema = RequestSchema.extend({
    body: z.object({
        paging: z.object({
          page: z.number().int(),
          limit: z.number().int(),
          sortBy: z.string().nonempty().refine(sortby => {
            const sortBy: string[] = Object.values(SortBy);
            return sortBy.includes(sortby);
          }),
          sortDirection: z.string().nonempty().refine(direction => {
        ...
        search: z.object({
          filters: z.array(
            z.object({
              targetFieldName: z.string().nonempty().refine(field => {
